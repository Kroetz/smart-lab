...
public class MeetingManagementMicroservice extends AbstractEntityManagementMicroservice<IMeeting, MeetingId> implements IMeetingManagementService {
    ...
    public void shortenMeeting(MeetingId meetingId, Duration shortening)
        ...
        try {
            this.meetingManagementApiClient.shortenMeeting(meetingId.getIdValue(), shortening.toMinutes());
        }
    ...
    public void extendMeeting(MeetingId meetingId, Duration extension) {
        try {
            this.meetingManagementApiClient.extendMeeting(meetingId.getIdValue(), extension.toMinutes());
        }
        catch(FeignException e) {
            if(e.status() == HttpStatus.NOT_FOUND.value()) {
                throw new EntityNotFoundException();
    ...
    public void shiftMeeting(MeetingId meetingId, Duration shift) {
        try {
            this.meetingManagementApiClient.shiftMeeting(meetingId.getIdValue(), shift.toMinutes());
        }
        catch(FeignException e) {
            if(e.status() == HttpStatus.NOT_FOUND.value()) {
                throw new EntityNotFoundException();
